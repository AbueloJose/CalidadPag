...
// Asegúrate de haber descargado FPDF y puesto en /lib/fpdf/
...
if (!isset($_SESSION['usuario_id']) || !in_array($_SESSION['usuario_rol'], ['admin', 'docente'])) {
    // Usamos 'die()' porque no queremos redirigir, solo mostrar un error
...
try {
    ...
    // Obtenemos estudiantes, su práctica, la empresa y el estado
    $stmt = $pdo->prepare("
        SELECT 
            u.nombres, 
            u.apellidos, 
    ...
    $pdf = new FPDF('P', 'mm', 'A4'); // P = Portrait (Vertical), mm = milímetros, A4
    $pdf->AddPage();
    
    // 7. Título del Reporte
    $pdf->SetFont('Arial', 'B', 16); // Fuente: Arial, Negrita, tamaño 16
    $pdf->Cell(0, 10, utf8_decode('Reporte de Prácticas Pre-Profesionales'), 0, 1, 'C'); // Título centrado
    ...
    $pdf->Cell(25, 8, 'Fecha Inicio', 1, 1, 'C', true); // 1 al final = Salto de línea
    ...
    // 10. Salida del PDF
    // 'D' fuerza la descarga con el nombre dado
